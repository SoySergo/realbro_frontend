...
// Единый URL бекенда для всех запросов (CORS на стороне бекенда)
const API_V1_BASE = (process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:8080').replace(/\/$/, '');
// Legacy fallback (тот же бекенд)
const API_BASE = `${API_V1_BASE}/api`;
